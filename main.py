...
async def get_fine_messages(channel):
    ...
    async for message in channel.history(limit=50):
        ...
        if not (message.author.bot
                ) and message.mentions:  # 봇이 작성하지 않은 멘션이 있는 메시지만 수집
...
async def get_unchecked_members(message):
    ...
    reaction = discord.utils.get(message.reactions,
                                 emoji="✅")  # 체크 이모지 객체 가져오기
    if reaction:
        # 체크 한 부원
        async for member in reaction.users():
            checked.append(member.id)

    unchecked = [member for member in mentioned
                 if member.id not in checked]  # 체크 안 한 부원
...
async def 벌금체크(ctx):
    ...
    if all_unchecked:
        ...
        await ctx.send(f"{mentions}\n이번 주 벌금 납부 안 하신 분들은 입금 후 체크 표시(✅) 꼭 남겨주세요!")
    else:
